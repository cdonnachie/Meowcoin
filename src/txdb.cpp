    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                POW_TYPE powType = POW_TYPE_MEOWPOW;
                if (IsLWMAActive(pindexNew->nHeight)) {
                    powType = static_cast<POW_TYPE>((pindexNew->nVersion >> 16) & 0xFF);
                    LogPrintf("Loading block %s with POW type %d\n", pindexNew->GetBlockHash().ToString(), powType);
                    if (!CheckProofOfWorkLWMA(pindexNew->GetBlockHash(), pindexNew->nBits, consensusParams, powType))
                        return error("%s: CheckProofOfWork failed: %s", __func__, pindexNew->ToString());
                } else {
                    if (!CheckProofOfWorkDGW(pindexNew->GetBlockHash(), pindexNew->nBits, consensusParams))
                        return error("%s: CheckProofOfWork failed: %s", __func__, pindexNew->ToString());
                }
