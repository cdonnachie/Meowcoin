...
bool ReadBlockFromDisk(CBlock& block, const uint32_t nHeight, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    block.SetNull();
    ...
    if (IsLWMAActive(nHeight)) {
        uint8_t powType = (block.nVersion >> 16) & 0xFF;
        if (powType == POW_TYPE_SCRYPT) {
            block.nHeight = nHeight;
        }
        if (!CheckProofOfWorkLWMA(block.GetHash(), block.nBits, consensusParams, powType == POW_TYPE_SCRYPT ? POW_TYPE_SCRYPT : POW_TYPE_MEOWPOW))
            return error("ReadBlockFromDisk: Errors in block LWMA header at %s", pos.ToString());
    } else {
        if (!CheckProofOfWorkDGW(block.GetHash(), block.nBits, consensusParams))
...
{
    if (!ReadBlockFromDisk(block, pindex->nHeight, pindex->GetBlockPos(), consensusParams)) {
        return false;
    }
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    ...
    // Apply version bits for deployments
...
public:
    ...
    {
        // Versionbits always active since Dual Mining and high bits repurposed at scrypt UASF activation;
        // So, don't use VERSIONBITS_TOP_MASK any time after LWMA activation.
        if (IsLWMAActive(pindex->nHeight)) {
            // For dual mining era, mask out POW type bits before checking version bits
            uint32_t maskedVersion = pindex->nVersion & 0xFF00FFFF;
            uint32_t expectedVersion = ComputeBlockVersion(pindex->pprev, params) & 0xFF00FFFF;

            return ((maskedVersion >> bit) & 1) != 0 &&
                   ((expectedVersion >> bit) & 1) == 0;
        } else {
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    if (pindexNew->nHeight == 0 || (pindexNew->pprev == chainActive.Tip())) {
    } else {
        // Handle mismatch case
        if (!chainActive.Tip()) {
            return state.DoS(0, error("%s: chainActive.Tip() is null", __func__),
                REJECT_INVALID, "null-chain-tip");
        }

        return state.DoS(0, error("%s: block doesn't connect to current tip", __func__),
            REJECT_INVALID, "tip-mismatch");
    }
...
{
    bool fCheckLWMAActive = IsLWMAActive(block.nHeight);
    ...
    if (fCheckPOW && block.nTime >= nKAWPOWActivationTime) {
        ...
        if (fCheckPOW && pcheckpoint && block.nHeight <= (uint32_t)pcheckpoint->nHeight) {
            if (fCheckLWMAActive) {
                uint8_t powType = (block.nVersion >> 16) & 0xFF;
                bool powResult = CheckProofOfWorkLWMA(block.GetHash(), block.nBits, consensusParams, powType == POW_TYPE_MEOWPOW ? POW_TYPE_MEOWPOW : POW_TYPE_SCRYPT);
                if (!powResult) {
                    return state.DoS(50, false, REJECT_INVALID, "high-hash", false, "proof of work failed");
                }
            } else {
                bool powResult = CheckProofOfWorkDGW(block.GetHash(), block.nBits, consensusParams);
                if (!powResult) {
                    return state.DoS(50, false, REJECT_INVALID, "high-hash", false, "proof of work failed");
                }
            }
    ...
    if (fCheckLWMAActive) {
        ...
        uint256 blockHashForPOW = block.GetHashFull(mix_hash);

        bool powResult = CheckProofOfWorkLWMA(blockHashForPOW, block.nBits, consensusParams, powType == POW_TYPE_MEOWPOW ? POW_TYPE_MEOWPOW : POW_TYPE_SCRYPT);

        if (fCheckPOW && !powResult) {
            return state.DoS(50, false, REJECT_INVALID, "high-hash", false, "proof of work failed");
        }
    } else {
        uint256 blockHashForPOW = block.GetHashFull(mix_hash);
        bool powResult = CheckProofOfWorkDGW(blockHashForPOW, block.nBits, consensusParams);
    ...
    if (fCheckPOW && block.nTime >= nKAWPOWActivationTime) {
        if (fCheckLWMAActive) {
            uint8_t powType = (block.nVersion >> 16) & 0xFF;
            if (powType == POW_TYPE_MEOWPOW && mix_hash != block.mix_hash) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (IsLWMAActive(nHeight)) {
        ...
        if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams, powType))
    ...
    if (IsDGWActive(pindexPrev->nHeight + 1)) {
        int64_t max_future_block_time = IsLWMAActive(pindexPrev->nHeight + 1) ? MAX_FUTURE_BLOCK_TIME_LWMA : MAX_FUTURE_BLOCK_TIME_DGW;
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (!ReadBlockFromDisk(block, pindex, chainparams.GetConsensus()))
            return error("VerifyDB(): *** ReadBlockFromDisk (Level 0) failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            CBlock block;
            if (!ReadBlockFromDisk(block, pindex, chainparams.GetConsensus()))
                return error("VerifyDB(): *** ReadBlockFromDisk (Level 4) failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
            ...
            try {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(*pblockrecursive, pblockrecursive->nHeight, it->second, chainparams.GetConsensus())) {
...
{
    return nBlockNumber >= GetParams().LWMAActivationBlock();
}
